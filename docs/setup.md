...
Diff context uses the selected model's available input window. The full diff
is included whenever it fits; only context-window overflow falls back to a
complete file overview plus large per-file slices.
